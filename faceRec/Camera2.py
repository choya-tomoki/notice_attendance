...
#名前変数
names=['none'] # TODO 名前を追加 
#画像の格納パス
pictpath='/home/pi/Documents/faceRec/picts'
#サーバ通知のURL
...
class Camera(BaseCamera):
    ...
    @staticmethod
    def moveDetect(img):
        global befImg, befTimes, attendance
        #入力画像をグレースケールに変換
    ...
    def faceDetect(img):
        ...
        for(x,y,w,h) in faces:
            ...
            if (confidence < 0 and confidence > 100):
                break
            if confidence > 85 and confidence < 100:
                #現在日付を取得
                nowstr=datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                nowTime=time.time()
                # 出席登録
                if names[no] not in attendance:
                    attendance.append(names[no])
                #ログ出力
